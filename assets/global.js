...
class SliderComponent extends HTMLElement {
  constructor() {
    ...
    // Autoplay feature to Slider component
    const speed = Number(this.slider.getAttribute('data-speed'));
    this.autoplayInterval = speed * 1000; // Adjust this value to set the autoplay interval in milliseconds
    this.autoplayTimer = null;
    if (this.slider.getAttribute('data-autoplay') === 'true') {
      this.startAutoplay();
    }
    // end of autoplay feature
  ...
  moveToNextSlide() {
    ...
    if (nextPosition >= this.sliderItemOffset * (this.totalPages - 1)) {
      // Reached the end, scroll back to the beginning
      this.setSlidePosition(0);
      console.log("Reched slider end");
    } else {
      this.setSlidePosition(nextPosition);
    }
  }

  // autoplay methods ends


  // Override the onButtonClick method to stop autoplay when manual navigation occurs
  onButtonClick(event) {
    event.preventDefault();
    this.stopAutoplay();
    const step = event.currentTarget.dataset.step || 1;
